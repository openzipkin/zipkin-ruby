...
describe ZipkinTracer::B3SingleHeaderFormat do
  ...
  context 'child span' do
    ...
    it 'has all fields' do
      expect(b3_single_header_format.trace_id).to eq('80f198ee56343ba864fe8b2a57d3eff7')
      expect(b3_single_header_format.span_id).to eq('e457b5a2e4d86bd1')
      expect(b3_single_header_format.parent_span_id).to eq('05e3ac9a4f6e3b90')
      expect(b3_single_header_format.sampled).to eq('1')
      expect(b3_single_header_format.flags).to eq(0)
  ...
  context 'sampled root span' do
    ...
    it 'does not have parent_span_id' do
      expect(b3_single_header_format.trace_id).to eq('80f198ee56343ba864fe8b2a57d3eff7')
      expect(b3_single_header_format.span_id).to eq('e457b5a2e4d86bd1')
      expect(b3_single_header_format.parent_span_id).to eq(nil)
      expect(b3_single_header_format.sampled).to eq('1')
      expect(b3_single_header_format.flags).to eq(0)
  ...
  context 'not yet sampled root span' do
    ...
    it 'does not have parent_span_id and sampled' do
      ...
      expect(b3_single_header_format.span_id).to eq('e457b5a2e4d86bd1')
      expect(b3_single_header_format.parent_span_id).to eq(nil)
      expect(b3_single_header_format.sampled).to eq(nil)
      expect(b3_single_header_format.flags).to eq(0)
  ...
  context 'debug RPC child span' do
    ...
    it 'has debug flag' do
      ...
      expect(b3_single_header_format.parent_span_id).to eq('05e3ac9a4f6e3b90')
      expect(b3_single_header_format.sampled).to eq(nil)
      expect(b3_single_header_format.flags).to eq(1)
      expect(b3_single_header_format.to_a).to eq(
  ...
  context 'do not sample flag only' do
    ...
    it 'has do not sample flag only' do
      expect(b3_single_header_format.trace_id).to eq(nil)
      expect(b3_single_header_format.span_id).to eq(nil)
      expect(b3_single_header_format.parent_span_id).to eq(nil)
      expect(b3_single_header_format.sampled).to eq('0')
      expect(b3_single_header_format.flags).to eq(0)
